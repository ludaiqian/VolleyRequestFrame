...
public class RequestQueue {
	 ...
	 */
	private final HashMap<Object, Request> mCurrentRequests = new HashMap<Object, Request>();
	...
	public void cancelAll(RequestFilter filter) {
		synchronized (mCurrentRequests) {
			for (Request<?> request : mCurrentRequests.values()) {
	...
	public void cancel(final Object key) {
		for (Request<?> request : mCurrentRequests.values()) {
	...
	public boolean add(Request request) {
		...
		// current requests.
		if (mCurrentRequests.containsKey(request.getKey()) && !request.canRepeated()) {
			//Logger.e("Refuse because duplicate request");
			return false;
		}
		request.setRequestQueue(this);
		synchronized (mCurrentRequests) {
			mCurrentRequests.put(request.getKey(), request);
		}
	...
	void finish(Request request) {
		...
		synchronized (mCurrentRequests) {
			mCurrentRequests.remove(request.getKey());
		}
